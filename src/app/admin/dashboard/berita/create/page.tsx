...
export default function CreateBeritaPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      alert("Simulasi berhasil mempublikasikan berita (UI Preview).");
    }, 1000);
  };
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="admin-panel-card" style={{ padding: "32px" }}>
        ...
        <div className="admin-form-group">
          ...
          <input type="text" id="judul" className="admin-input" placeholder="Masukkan judul yang menarik..." required />
        </div>

        <div className="admin-form-grid">
          <div className="admin-form-group">
            <label className="admin-label" htmlFor="tanggal">Tanggal Publikasi</label>
            <input type="date" id="tanggal" className="admin-input" defaultValue={new Date().toISOString().split('T')[0]} />
          </div>

          <div className="admin-form-group">
            <label className="admin-label" htmlFor="foto">Foto Utama (Hero Image) *</label>
            <input type="file" id="foto" className="admin-input" accept="image/*" style={{ padding: "10px 16px" }} required />
          </div>
        </div>
        ...
        <div className="admin-form-group">
          ...
          <p style={{ fontSize: "12px", color: "var(--muted)", marginTop: "8px" }}>
            *Ini adalah tampilan UI untuk editor teks. Integrasi penyimpanan Teks Kaya (Rich Text) akan dilakukan di tahap backend.
          </p>
        </div>
